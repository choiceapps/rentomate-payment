...
const EditRent = () => {
    ...
    const {rentCost, montOfRent, relentedCost, collectRent, securityDeposit, protectRent} = data;
    ...
    const handleSubmit = async (e) => {
      setLoading(true)
      e.preventDefault();
      await axios.put(`http://localhost:4000/rent/${newdata._id}`, data).then((res) => {
    ...
    <div className={custom.rentCont} >
      ...
      <form onSubmit={handleSubmit} className={custom.form}>
        <div className={Style.rentInfo}>
            ...
            <input type="number" name="rentCost" id="rentCost" placeholder="Enter amount" value={rentCost} onChange={(e) => handleChange(e)}/>
        </div>
        ...
        <button type="submit" className={Style.button}>{loading ? "Loading..." : "Submit"}</button>
      </form>
    </div>
  </div>
    );
